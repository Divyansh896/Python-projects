...
mixer.music.load('gallery/audio/background.wav')
mixer.music.play(-1)
...
background=pygame.image.load('gallery/sprites/bg.png')

spaceshipimg=pygame.image.load('gallery/sprites/playership1.png')
...
for i in range(no_of_aliens):
    ...
    alienimg.append(pygame.transform.rotate(pygame.image.load('gallery/sprites/alienship.png'),180))
...
check=False
bulletX=400
bulletY=spaceshipimg.get_width()/2
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type==pygame.KEYDOWN:
            ...
            if event.key==pygame.K_SPACE:
                if check is False:
                    bulletSound=mixer.Sound('gallery/audio/laser.wav')
                    bulletSound.play()
                    check=True
                    bulletX=spaceshipX+35
    ...
    for i in range(no_of_aliens):
        ...
        if distance < 70:
            explosion= mixer.Sound('gallery/audio/explosion.wav')
            explosion.play()
            bulletY = 480
            check = False
